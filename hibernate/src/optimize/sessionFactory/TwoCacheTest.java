package optimize.sessionFactory;
...
public class TwoCacheTest {
	...
	static{
		...
		//加载配置文件
		configuration.configure("optimize/sessionFactory/hibernate.cfg.xml");
	...
	public void testGet(){
		...
		classes = (Classes)session.get(Classes.class, 2L);//没有发出SQL语句，在二级缓存中取
	...
	public void testSave(){
		...
		transaction.commit();//提交可是临时对象保存为持久化对象，保存到二级缓存
//		session.flush();
		session.close();//关闭不能提交事务
		session = sessionFactory.openSession();
		classes = (Classes)session.get(Classes.class, classes.getCid());//从二级缓存中获取，没有发出SQL语句
		session.close();
	}
	/**
	 * 把溢出的数据存放到磁盘上
	...
	public void test_list_iterate(){
		...
		// select classes0_.cid as col_0_0_ from Classes classes0_只查出id；
//		Iterator<Classes> iterators=query.iterate();
//		while (iterators.hasNext()) {
//			Classes classes = (Classes) iterators.next();
//			System.out.println(classes.getCname());
//		}
		session.close();
	}

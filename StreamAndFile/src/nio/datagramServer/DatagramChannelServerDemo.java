...
public class DatagramChannelServerDemo {
	...
	public DatagramChannelServerDemo(int port) throws IOException {
		try {
			this.port = port;
	...
	public void service() throws IOException {
	 ...
	 public synchronized void reveice(SelectionKey key) throws IOException {
		if (key == null)
			return;
		...
		System.out.println("ip:"+clientAddress+";port:"+clientPost);
		
		buf.flip(); // make buffer ready for read
		while (buf.hasRemaining()) {
			buf.get(new byte[buf.limit()]);// read 1 byte at a time
			content += new String(buf.array(),"GBK");
		}
		...
		buf2.clear();
		buf2.put(("消息推送内容 abc..UDP是一个非连接的协议，"
				      + "传输数据之前源端和终端不建立连接，当它想传送时就简单地去抓取来自应用程序的数据，"
				      + "并尽可能快地把它扔到网络上。"
				      + "在发送端UDP是一个非连接的协议，传输数据之前源端和终端不建立连接，"
	...
	public static void main(String[] args) throws IOException {
		new DatagramChannelServerDemo(8999).service();
	}
}

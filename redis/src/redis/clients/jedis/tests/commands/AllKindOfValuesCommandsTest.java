...
public class AllKindOfValuesCommandsTest extends JedisCommandTestBase {
  ...
  public void existsMany() {
    ...
    long reply = jedis.exists("foo1", "foo2","foo3");
  ...
  public void renameOldAndNewAreTheSame() {
    jedis.set("foo", "bar");
    jedis.set("foo1", "bar1");
    jedis.rename("foo", "foo1");
    System.out.println(jedis.get("foo1"));//bar
  ...
  public void renamenx() {
    ...
    System.out.println("jedis.get(\"bar\")="+jedis.get("bar"));//bar,此时bar已经存在，
    System.out.println("jedis.get(\"foo\")="+jedis.get("foo"));//null
  ...
  public void persist() {
    ...
    assertEquals(1, status);
    jedis.set("foo1", "v111");
    System.out.println(jedis.persist("foo1"));//0
    assertEquals(0, jedis.persist("foo1").longValue());
  ...
  @Test
  public void pexpire() {
    long status = jedis.expire("foo", 10000);
    assertEquals(0, status);

    jedis.set("foo1", "bar1");
    status = jedis.expire("foo1", 10000);
    assertEquals(1, status);

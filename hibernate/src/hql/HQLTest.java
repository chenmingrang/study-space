...
public class HQLTest {
	...
	static{
		...
		//加载配置文件
		configuration.configure("hql/hibernate.cfg.xml");
	...
	public void testClasses_Select_Constructor(){
		Session session = sessionFactory.openSession();
//		List classesList = session.createQuery("select new Classes(cid,cname) from Classes").list();
		List classesList = session.createQuery("select new map(cid as id,cname as name) from Classes").list();
		//[{id=1, name=IT1班}, {id=2, name=IT二班}, {id=3, name=IT三班}]
		classesList.size();
		session.close();
	}
	...
	public void testSelect_Student_Classes_EQ_FETCH(){
		Session session = sessionFactory.openSession();
		List studentList = session.createQuery("from Student s inner join fetch s.classes c").list();
	...
	public void testSelect_Constructor_Classes_Student_Inner(){
		Session session = sessionFactory.openSession();
		List classesList = session.createQuery("select new hql.ClassesView(c.cname,s.sname) " +

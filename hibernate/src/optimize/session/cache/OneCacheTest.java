package optimize.session.cache;

import java.util.Set;

import optimize.session.Classes;
import optimize.session.Student;
...
public class OneCacheTest {
	...
	static{
		...
		//加载配置文件
		configuration.configure("optimize/session/hibernate.cfg.xml");
	...
	public void testSave(){
		...
		classes.setCname("军事研习班");
		classes.setDescription("军事人才的摇篮");
		session.save(classes);
		Classes classes2 = (Classes)session.get(Classes.class, classes.getCid());//没有发出 select语句，对象存在缓存中
		transaction.commit();
	}
	...
	public void testUpdate(){
		...
		session.update(classes);//缓存被清空，没有发出update语句
	...
	public void testFlush(){
		...
		student.setSname("刘少奇");
		student.setDescription("副主席");
		Student student2 = new Student();
		student2.setSname("刘源");
		student2.setDescription("刘少奇的儿子");
